...
class ThemeDomainServiceTest {
    ...
    private ReservationTimeDomainService reservationTimeDomainService;

    @BeforeEach
    void setUp() {
        themeDomainService = new ThemeDomainService(themeRepository, reservationRepository);
        ...
        reservationTimeDomainService = new ReservationTimeDomainService(reservationTimeRepository,
                reservationRepository);
    }

    @Test
    void delete() {
        ThemeResponse themeResponse = themeDomainService.create(
                new ThemeCreateRequest("논리", "논리 게임 with Vector", "image.png"));
        themeDomainService.delete(themeResponse.id());
        assertThat(themeDomainService.getThemes().size()).isEqualTo(0);
    }
    ...
    void getPopularThemes() {
        ...
        ThemeResponse themeResponse1 = themeDomainService.create(
                new ThemeCreateRequest("추리", "셜록 홈즈: 실종된 보석의 비밀", "sherlock_jewel.png"));
        ThemeResponse themeResponse2 = themeDomainService.create(
                new ThemeCreateRequest("논리", "양자 코드: 암호 해독 게임", "quantum_code.png"));

        ThemeResponse themeResponse3 = themeDomainService.create(
                new ThemeCreateRequest("공포", "저주받은 병원: 13호실의 비밀", "cursed_hospital.png"));
        ThemeResponse themeResponse4 = themeDomainService.create(
                new ThemeCreateRequest("모험", "잃어버린 문명: 마야의 유산", "maya_civilization.png"));

        ThemeResponse themeResponse5 = themeDomainService.create(
                new ThemeCreateRequest("판타지", "드래곤의 동굴: 마법의 보물", "dragon_cave.png"));
        ThemeResponse themeResponse6 = themeDomainService.create(
                new ThemeCreateRequest("역사", "타임머신: 조선 왕조의 비밀", "joseon_dynasty.png"));

        ThemeResponse themeResponse7 = themeDomainService.create(
                new ThemeCreateRequest("SF", "화성 기지: 레드 플래닛의 음모", "mars_base.png"));
        ThemeResponse themeResponse8 = themeDomainService.create(
                new ThemeCreateRequest("스릴러", "연쇄 살인마의 게임", "serial_killer_game.png"));

        ThemeResponse themeResponse9 = themeDomainService.create(
                new ThemeCreateRequest("코미디", "유머 수사대: 웃음을 찾아서", "humor_detective.png"));

        ThemeResponse themeResponse10 = themeDomainService.create(
                new ThemeCreateRequest("액션", "비밀 요원: 마지막 미션", "secret_agent.png"));
        ThemeResponse themeResponse11 = themeDomainService.create(
                new ThemeCreateRequest("신비", "신화의 미궁: 그리스 신들의 시험", "greek_gods.png"));
        ThemeResponse themeResponse12 = themeDomainService.create(
                new ThemeCreateRequest("음악", "멜로디 탐정: 잃어버린 노래", "lost_melody.png"));

        ReservationTimeResponse reservationTime1 = reservationTimeDomainService.create(
                new ReservationTimeCreateRequest(LocalTime.of(10, 0)));
        ReservationTimeResponse reservationTime2 = reservationTimeDomainService.create(
        ...
        List<ThemeResponse> popularThemes = themeDomainService.getPopularThemes(7, 10);
