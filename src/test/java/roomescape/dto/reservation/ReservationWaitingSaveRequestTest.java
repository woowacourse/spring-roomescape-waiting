...
class ReservationWaitingSaveRequestTest {

    @ParameterizedTest
    @NullSource
    @ValueSource(strings = {"", "22:00:00", "abc"})
    @DisplayName("예약 대기 날짜 입력 값이 유효하지 않으면 예외가 발생한다.")
    void throwExceptionWhenCannotConvertToLocalDate(final String invalidDate) {
        final ReservationWaitingSaveRequest request = new ReservationWaitingSaveRequest(invalidDate, 1L, 1L);
