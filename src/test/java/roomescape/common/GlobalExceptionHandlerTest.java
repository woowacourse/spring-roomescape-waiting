...
class GlobalExceptionHandlerTest {
    ...
    void handle_illegal_argument_exception() {
        ResponseEntity<String> response = globalExceptionHandler.handleIllegalArgumentException();
    ...
    void handle_null_pointer_exception() {
        ResponseEntity<String> response = globalExceptionHandler.handleNullPointerException();

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
    }

    @DisplayName("CustomException이 발생하면 400 Bad Request와 함께 메시지를 반환한다.")
    @Test
