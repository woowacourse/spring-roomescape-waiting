...
class ReservationTimeServiceTest {
    ...
    void 정상적인_시간_삭제는_성공해야_한다() {
        ReservationTime time = RoomEscapeFixture.TIME;
        given(reservationTimeRepository.findById(1L)).willReturn(Optional.of(time));
        given(slotRepository.findByTime(time)).willReturn(Optional.empty());
    ...
    void 예약이_있는_시간_삭제시_예외가_발생한다() {
        ...
        given(reservationTimeRepository.findById(1L)).willReturn(Optional.of(time));
        given(slotRepository.findByTime(time)).willReturn(Optional.of(slot));
