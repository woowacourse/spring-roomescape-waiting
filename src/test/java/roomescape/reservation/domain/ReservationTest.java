...
class ReservationTest {
    ...
    @DisplayName("이름이 같다면 예외를 발생하지 않는다")
    @Test
    void validateOwnedBy_이름이_같다면_정상() {
        // given
        ReservationTime time = new ReservationTime(1L, LocalDateTime.now(), LocalDateTime.now().plusHours(2));
        Theme theme = new Theme("라이의 공포 방", "라이라이차라차", "test-url");
        Reservation reservation = new Reservation("라이",
        ...
        // when & then
        assertThatCode(() -> reservation.validateOwnedBy("라이"))
    ...
    @DisplayName("예약 상태가 RESERVED가 아니면 false를 반환한다")
    @Test
    void isReserved_false_반환_테스트() {
        // given
        ReservationTime time = new ReservationTime(1L, LocalDateTime.now(), LocalDateTime.now().plusHours(2));
        Theme theme = new Theme("라이의 공포 방", "라이라이차라차", "test-url");
        Reservation reservation = new Reservation("라이",
