...
class AdminWaitingServiceTest {
    ...
    void 특정_회원의_예약_대기를_추가한다() {
        ...
        createReservation(date, time, theme, member2); // member2 예약 추가
    ...
    void 동일한_회원의_예약이_이미_있는_경우_예약_대기를_추가할_수_없다() {
        ...
        createReservation(date, time, theme, member); // member 예약 추가
    ...
    void 예약_대기를_삭제한다() {
        ...
        createReservation(date, time, theme, member2); // member2 예약

        final WaitingResponse waitingResponse = adminWaitingService.create(request); // member1 예약 대기 추가

        // when
        adminWaitingService.deleteAsAdmin(waitingResponse.id()); // member1 예약 대기 삭제

        // then
        Assertions.assertThat(waitingRepository.findById(waitingResponse.id()).isEmpty())
                .isTrue();
    }
    ...
    void 예약_대기_목록_전체를_조회한다() {
        ...
        createReservation(date, time, theme, members.get(0)); // 0번 member 예약 추가
