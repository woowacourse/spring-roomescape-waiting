...
class PaymentCommandServiceTest {
    ...
    void confirm_payment() {
        ...
        SoftAssertions.assertSoftly(softly -> {
            ...
            softly.assertThat(testHelper.findPaymentKey(order.getOrderId().value())).isEqualTo(PAYMENT_KEY);
            softly.assertThat(confirmation.paymentKey()).isEqualTo(PAYMENT_KEY);
            softly.assertThat(confirmation.orderId()).isEqualTo(order.getOrderId().value());
            softly.assertThat(confirmation.amount()).isEqualTo(order.getAmount().value());
        });
    }
    ...
    private PaymentConfirmCommand paymentConfirmCommand(PaymentOrder order) {
        return paymentConfirmCommand(order, order.getAmount().value());
    }

    private PaymentConfirmCommand paymentConfirmCommand(PaymentOrder order, Long amount) {
        return new PaymentConfirmCommand(PAYMENT_KEY, order.getOrderId().value(), amount);
    }
}
