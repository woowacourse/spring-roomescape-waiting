 ...
 * 사용자 관점의 흐름 검증이다.
 *
 * <p>의도적으로 적게 둔다. 대기 신청의 분기들(예약 없는 슬롯 거부, 중복 거부 등)은
 * WaitingServiceTest가 이미 검증했고, 순번 재정렬 규칙은 WaitingsTest가 검증했다.
 * 여기서 같은 것을 다시 검증하면 "같은 것을 두 번 검증한다"는 금지에 걸린다.
 * 인수 테스트는 "그 규칙들이 실제 HTTP 흐름에 연결되어 사용자에게 도달하는가"의 대표 경로만 본다.
 *
