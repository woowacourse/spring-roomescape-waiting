 ...
 * 요청은 토큰 1개를 소비하고, 없으면 거부된다.
 *
 * <p>시계를 {@link LongSupplier} 로 주입받아, 테스트에서 가짜 시계로 결정적으로 검증할 수 있다.
 * 들어오는(inbound)·나가는(outbound) 양방향에서 같은 알고리즘으로 재사용된다.
 */
public class TokenBucketRateLimiter {
    ...
    /**
     * 토큰이 있으면 1개 소비하고 true, 없으면 false. 동시 요청에서도 정확히 capacity 개만 통과하도록 동기화한다.
     */
    ...
    /**
     * 다음 요청이 통과 가능해질 때까지 필요한 대기 시간(초). 토큰이 충분하면 0, 아니면 1개가 찰 때까지 걸리는 시간을 올림한다.
     */
