...
public class Member {
    ...
    public Member(Long id, String loginId, String name, String password, Role role) {
        ...
        require(validatedLoginId.length() <= MAX_LOGIN_ID_SIZE, INVALID_INPUT, "로그인 ID는 255자를 초과할 수 없습니다.");
        require(validatedName.length() <= MAX_NAME_SIZE, INVALID_INPUT, "회원 이름은 10자를 초과할 수 없습니다.");
        require(validatedPassword.length() <= MAX_PASSWORD_SIZE, INVALID_INPUT, "비밀번호는 255자를 초과할 수 없습니다.");
    ...
    public void validateSameMember(Member member) {
        if (member == null || !Objects.equals(id, member.id)) {
