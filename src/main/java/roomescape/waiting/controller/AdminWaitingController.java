...
public class AdminWaitingController {
    ...
    @PostMapping("/{id}")
    public ResponseEntity<AdminWaitingUpdateResponse> updateWaitingReservation(
            @RequestBody AdminWaitingUpdateRequest request,
            @PathVariable("id") Long waitingId,
            Member member) {
        member.validateAdminOrThrow();
        AdminWaitingUpdateResponse response = waitingService.UpdateWaitingReservation(request, waitingId);
        return ResponseEntity.ok(response);
    }
}
