...
public class TossPaymentException extends RuntimeException {
    ...
    public static TossPaymentException of(HttpStatusCode status, TossErrorResponse errorResponse) {
        return switch (errorResponse.code()) {
            case "ALREADY_PROCESSED_PAYMENT" -> new AlreadyProcessed(errorResponse.message());
            case "DUPLICATED_ORDER_ID" -> new DuplicatedOrder(errorResponse.message());
            case "NOT_FOUND_PAYMENT_SESSION" -> new SessionExpired(errorResponse.message());
            case "INVALID_REQUEST" -> new InvalidRequest(errorResponse.message());
            case "UNAUTHORIZED_KEY", "INVALID_API_KEY" -> new GatewayConfig(errorResponse.message());
            case "REJECT_CARD_PAYMENT" -> new CardRejected(errorResponse.message());
            case "NOT_FOUND_PAYMENT" -> new PaymentNotFound(errorResponse.message());
            case "FAILED_PAYMENT_INTERNAL_SYSTEM_PROCESSING" -> new Retryable(errorResponse.message());
            default -> new TossPaymentException(status, errorResponse.code(), errorResponse.message());
        };
    }
