...
public class JdbcWaitRepository implements WaitRepository {
    ...
    public Wait save(Wait waitWithoutId) {
        ...
        jdbcTemplate.update(connection -> {
            ...
            preparedStatement.setObject(1, waitWithoutId.getCreatedAt());
            preparedStatement.setString(2, waitWithoutId.getName());
            preparedStatement.setDate(3, Date.valueOf(waitWithoutId.getReservationDate()));
            preparedStatement.setLong(4, waitWithoutId.getTime().getId());
            preparedStatement.setLong(5, waitWithoutId.getTheme().getId());

            return preparedStatement;
        }, keyHolder);

        Long id = keyHolder.getKey().longValue();
        return waitWithoutId.withId(id);
    }
    ...
    @Override
    public Waits findBySlot(LocalDate date, Long timeId, Long themeId) {
        String sql = """
                SELECT w.id, w.created_at, w.name, w.reservation_date,
        ...
        return new Waits(
                jdbcTemplate.query(sql, waitDetailDtoRowMapper(),
                        date,
                        timeId,
                        themeId)
        );
    ...
    public Waits findByName(String name) {
        ...
        return new Waits(
                jdbcTemplate.query(sql, waitDetailDtoRowMapper(), name)
        );
    }
    ...
    public Long findOrderByWait(Wait wait) {
        String sql = """
                ...
                SELECT `order` FROM slot_waiting_list WHERE `id` = ?
                """;
    ...
    private static RowMapper<Wait> waitDetailDtoRowMapper() {
        return (resultSet, rowNum) -> {
            ...
            return new Wait(id, createdAt, name, new Slot(date, reservationTime, theme));
        };
    }
}
