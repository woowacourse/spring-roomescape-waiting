...
public class ReservationService {
  ...
  public ReservationCreateResponse create(ReservationRequest request) {
    ...
    boolean isExistSlot = reservationDao.findByDateTimeThemeStatus(request.date(), request.timeId(),
        request.themeId());

    boolean isAlreadyExist = reservationDao.findByNameAndDateAndTimeAndTheme(request.name(),
    ...
    Reservation reservation = reservationDao.insert(request.name(), LocalDate.parse(request.date()),
        request.timeId(), request.themeId(), status);
  ...
  public void promoteFirstWaiting(Reservation reservation) {
    boolean slotTaken = reservationDao.findByDateTimeThemeStatus(
        reservation.getDate().toString(), reservation.getTime().getId(),
  ...
  @Transactional(isolation = Isolation.READ_COMMITTED)
  public Reservation updateMyReservation(UpdateMyReservation updateMyReservation, String name,
      Long reservationId) {
    Reservation reservation = reservationDao.findById(reservationId);
    validateReservationAuthority(name, reservation);
    isReservationExists(updateMyReservation.date(), updateMyReservation.timeId(),
        reservation.getTheme().getId());
    reservationDao.updateReservation(updateMyReservation.date(), updateMyReservation.timeId(), name,
        reservationId);
