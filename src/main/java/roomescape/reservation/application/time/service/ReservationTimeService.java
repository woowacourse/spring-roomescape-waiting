package roomescape.reservation.application.time.service;
...
public class ReservationTimeService {
    ...
    public ReservationTimeInfo createReservationTime(final ReservationTimeCreateCommand command) {
        ...
        final ReservationTime reservationTime = command.convertToEntity();
    ...
    public List<ReservationTimeAvailabilityInfo> findAvailableTimes(final LocalDate date, final long themeId) {
        final List<ReservationTime> reservationTimes = reservationTimeRepository.findAll();
        final List<Reservation> reservations = reservationRepository.findAllByDateAndThemeId(date, themeId);
        return reservationTimes.stream()
                .map(time ->
                        new ReservationTimeAvailabilityInfo(time.id(), time.startAt(), isAlreadyBooked(time, reservations)))
                .toList();
    }
