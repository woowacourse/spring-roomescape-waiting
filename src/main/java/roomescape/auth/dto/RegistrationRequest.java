...
public record RegistrationRequest(
        ...
        @NotBlank(message = "[ERROR] 이메일 형식으로 입력해 주세요.") String email,
        @NotBlank(message = "[ERROR] 비밀번호가 없습니다.")
        @Size(min = 8, max = 50, message = "[ERROR] 비밀번호는 8자 이상, 50자 이하여야 합니다.") String password
) {

    public Member createMember() {
        return new Member(null, name, new roomescape.member.domain.Email(email), new Password(password), Role.USER);
    }
}
