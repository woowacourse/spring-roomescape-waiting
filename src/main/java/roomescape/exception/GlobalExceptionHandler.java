...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleInvalidClientField(InvalidClientFieldException ex,
    ...
    public ResponseEntity<ErrorResponse> handleInvalidClientFieldWithValue(InvalidClientFieldWithValueException ex,
                                                                           HttpServletRequest req) {
        final String message = MESSAGE_FORMAT_WITH_VALUE.formatted(ex.getErrorType().getMessage(), ex.getFieldName(),
                ex.getValue());
        final ErrorResponse errorResponse = new ErrorResponse(HttpStatus.BAD_REQUEST, req.getRequestURI(), message);
        return ResponseEntity.badRequest().body(errorResponse);
    }
    ...
    public Object handleMissingServletRequestParameterException(
        ...
        final String message = MESSAGE_FORMAT
                .formatted(ErrorType.EMPTY_VALUE_NOT_ALLOWED.getMessage(), ex.getParameterName());
        final ErrorResponse errorResponse = new ErrorResponse(HttpStatus.BAD_REQUEST, req.getRequestURI(), message);
        return ResponseEntity.badRequest().body(errorResponse);
    }
