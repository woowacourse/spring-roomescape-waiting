...
import roomescape.payment.exception.PaymentOwnerMismatchException;
...
public class PaymentService {
    ...
    public PaymentCheckout prepareReservation(
        ...
        paymentOrderDao.insert(PaymentOrder.ready(orderId, saved.getId(), reservationAmount));
    ...
    public PaymentResult confirm(Long memberId, String paymentKey, String orderId, Long amount) {
        ...
        paymentOrderDao.update(order.complete(result.paymentKey()));
    ...
    public void fail(Long memberId, String orderId) {
        ...
        PaymentOrder order = paymentOrderDao.findByOrderId(orderId).orElse(null);
        if (order == null || order.isDone()) {
            return;
        }
        Reservation reservation = findReservation(order.getReservationId());
        validateOwner(reservation, memberId);
        paymentOrderDao.deleteByOrderId(orderId);
    ...
    private PaymentOrder findPaymentOrder(String orderId) {
        return paymentOrderDao.findByOrderIdForUpdate(orderId)
