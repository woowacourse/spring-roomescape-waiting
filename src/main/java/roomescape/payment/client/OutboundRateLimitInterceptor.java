...
/**
 * 나가는(outbound) 토스 호출에 거는 Rate Limit 인터셉터(클라이언트 관점).
 *
 * <p>들어오는 쪽(서버 관점)과 똑같은 {@link TokenBucketRateLimiter} 를 방향만 바꿔 재사용한다.
 * 외부 호출 '전에' 토큰을 소비해, 한도를 넘으면 보내지 않고 {@link OutboundRateLimitException} 으로 거부한다.
 */
