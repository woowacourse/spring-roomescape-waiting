...
public class ReservationSpecs {
    ...
    public static Specification<Reservation> byThemeId(final long id) {
        return (root, query, criteriaBuilder) ->
            criteriaBuilder.equal(root.get("theme").get("id"), id);
    }
    ...
    public static Specification<Reservation> byFilter(final ReservationSearchFilter filter) {
        return (root, query, cb) -> {
            ...
            if (filter.dateFrom() != null) {
                predicates.add(cb.greaterThanOrEqualTo(root.get("dateTime").get("date"), filter.dateFrom()));
            }
            if (filter.dateTo() != null) {
                predicates.add(cb.lessThanOrEqualTo(root.get("dateTime").get("date"), filter.dateTo()));
            }
