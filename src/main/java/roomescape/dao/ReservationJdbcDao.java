...
public class ReservationJdbcDao implements ReservationDao {
    ...
    public List<Reservation> findAll(ReservationSearchDto reservationSearchDto) {
        WhereClause whereClause = toWhereClause(reservationSearchDto);
    ...
    private WhereClause toWhereClause(ReservationSearchDto reservationSearchDto) {
        List<String> clauses = new ArrayList<>();
        List<Object> params = new ArrayList<>();

        if (reservationSearchDto.themeId() != null) {
            clauses.add("theme_id = ?");
            params.add(reservationSearchDto.themeId());
        }

        if (reservationSearchDto.memberId() != null) {
            clauses.add("member_id = ?");
            params.add(reservationSearchDto.memberId());
        }

        if (reservationSearchDto.startDate() != null) {
            clauses.add("r.date >= ?");
            params.add(reservationSearchDto.startDate());
        }

        if (reservationSearchDto.endDate() != null) {
            clauses.add("r.date <= ?");
            params.add(reservationSearchDto.endDate());
        }
