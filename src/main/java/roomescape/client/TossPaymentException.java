...
public class TossPaymentException extends RuntimeException {
    ...
    public static TossPaymentException of(HttpStatusCode status, TossErrorResponse error) {
        ...
        return switch (errorCode) {
            case "ALREADY_PROCESSED_PAYMENT" -> new TossPaymentException.AlreadyProcessed(errorMessage);
            case "DUPLICATED_ORDER_ID" -> new TossPaymentException.DuplicatedOrder(errorMessage);
            case "NOT_FOUND_PAYMENT_SESSION" -> new TossPaymentException.SessionExpired(errorMessage);
            case "INVALID_REQUEST" -> new TossPaymentException.InvalidRequest(errorMessage);
            case "UNAUTHORIZED_KEY" -> new TossPaymentException.GatewayConfig(errorMessage);
            case "INVALID_API_KEY" -> new TossPaymentException.GatewayConfig(errorMessage);
            case "REJECT_CARD_PAYMENT" -> new TossPaymentException.CardRejected(errorMessage);
            case "NOT_FOUND_PAYMENT" -> new TossPaymentException.PaymentNotFound(errorMessage);
            case "FAILED_PAYMENT_INTERNAL_SYSTEM_PROCESSING" -> new TossPaymentException.Retryable(errorMessage);
