...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document
        ...
        .addEventListener("click", onReservationButtonClick);
    document
        .getElementById('wait-button')
        .addEventListener('click', onWaitButtonClick);
});
...
function createSlot(type, text, id, booked) {
    ...
    div.className =
        type + "-slot cursor-pointer bg-light border rounded p-3 mb-2";
...
function fetchAvailableTimes(date, themeId) {
    ...
    TODO: [3단계] 사용자 예약 - 예약 가능 시간 조회 API 호출
          요청 포맷에 맞게 설정
    */
    fetch(`/times/available-times?date=${date}&themeId=${themeId}`, {
        // 예약 가능 시간 조회 API endpoint
        method: "GET",
        headers: {
...
function renderAvailableTimes(times) {
    ...
    if (times.length === 0) {
        timeSlots.innerHTML =
            '<div class="no-times">선택할 수 있는 시간이 없습니다.</div>';
        return;
    }
    times.forEach((time) => {
        /*
...
function onReservationButtonClick() {
    const selectedDate = document.getElementById("datepicker").value;
    const selectedThemeId = document
        .querySelector(".theme-slot.active")
