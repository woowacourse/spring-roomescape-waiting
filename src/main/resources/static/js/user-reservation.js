...
function fetchAvailableTimes(date, themeId) {
    const url = `/times/possible?date=${encodeURIComponent(date)}&themeId=${encodeURIComponent(themeId)}`;
...
function checkDateAndThemeAndTime() {
    ...
    if (selectedDate && selectedThemeElement && selectedTimeElement) {
        if (selectedTimeElement.getAttribute('data-time-booked') === 'true') {
            // 선택된 시간이 이미 예약된 경우
            reserveButton.classList.add("disabled"); // 예약 버튼 비활성화
            waitButton.classList.remove("disabled"); // 예약 대기 버튼 활성화
        } else {
            // 선택된 시간이 예약 가능한 경우
            reserveButton.classList.remove("disabled"); // 예약 버튼 활성화
