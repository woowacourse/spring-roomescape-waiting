...
function fetchAvailableTimes(date, themeId) {
    /*
    사용자 예약 - 예약 가능 시간 조회 API 호출
          요청 포맷에 맞게 설정
    */
    let url = '/availableTimes?date=' + date + '&themeId=' + themeId;
...
function renderAvailableTimes(times) {
    ...
    times.forEach(time => {
        /*
        사용자 예약 - 예약 가능 시간 조회 API 호출 후 렌더링
              response 명세에 맞춰 createSlot 함수 호출 시 값 설정
        */
...
function onReservationButtonClick() {
    ...
    if (selectedDate && selectedThemeId && selectedTimeId) {
        ...
        fetch('/reservations', {
            ...
            .catch(error => {
                alert("예약 신청 과정 중 오류가 발생했습니다!");
                console.error(error);
            });
    } else {
        alert("예약을 신청하기 전에 날짜, 테마, 시간을 선택해주세요.");
    }
}
...
function onWaitButtonClick() {
    ...
    if (selectedDate && selectedThemeId && selectedTimeId) {
        ...
        fetch('/waitings', {
            ...
            .catch(error => {
                alert("예약 대기 신청 과정 중 오류가 발생했습니다!");
                console.error(error);
            });
    } else {
