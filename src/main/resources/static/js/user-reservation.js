...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('reserve-waiting-button').addEventListener('click', onReservationWaitingButtonClick);
});
...
function checkDateAndThemeAndTime() {
    ...
    const reserveWaitingButton = document.getElementById("reserve-waiting-button");

    if (selectedDate && selectedThemeElement && selectedTimeElement) {
        if (selectedTimeElement.getAttribute('data-time-booked') === 'true') {
            // 선택된 시간이 이미 예약된 경우
            reserveButton.classList.add("disabled");
            reserveWaitingButton.classList.remove("disabled");
        } else {
            // 선택된 시간이 예약 가능한 경우
            reserveButton.classList.remove("disabled");
            reserveWaitingButton.classList.add("disabled");
        }
    } else {
...
function onReservationWaitingButtonClick() {
    const selectedDate = document.getElementById("datepicker").value;
    const selectedThemeId = document.querySelector('.theme-slot.active')?.getAttribute('data-theme-id');
    const selectedTimeId = document.querySelector('.time-slot.active')?.getAttribute('data-time-id');
