...
function render(data) {
    ...
    data.forEach(item => {
        ...
        const theme = item.slot.theme.name.value;
        const date = item.slot.date.value;
        const time = item.slot.time.value;
        const status = item.sequence === 1 ? '예약' : item.sequence + '번째 예약 대기';
