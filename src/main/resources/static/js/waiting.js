const waitsEndPoint = "/admin/waits";


document.addEventListener('DOMContentLoaded', () => {
    // Wait [4단계] 예약 대기 관리 기능. 예약 대기 목록 조회 endpoint 설정
    fetch(waitsEndPoint) // 내 예약 목록 조회 API 호출
        .then(response => {
...
function render(data) {
    ...
    data.forEach(item => {
        ...
        // Wait [4단계] 예약 대기 관리 기능. 예약 대기 목록 조회 response 명세에 맞춰 값 설정
        const id = item.id;
