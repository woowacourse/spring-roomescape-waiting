...
- [x] 예약 시간을 삭제한다.
    ...
    - [x] [예외] 예약이 존재하는 시간을 삭제하는 경우, 409를 반환한다.
...
- [x] 테마를 삭제한다.
    ...
    - [x] [예외] 예약이 존재하는 테마를 삭제하는 경우, 409를 반환한다.
...
- [x] 예약 전체 목록을 조회한다.
- [x] 예약자 이름으로 본인의 예약 및 대기 목록을 조회한다.
    - [x] 각 항목에 예약 상태(RESERVED / WAITING)와 대기 순번을 함께 반환한다.
- [x] 예약 날짜·시간을 변경한다.
    - [x] [예외] 존재하지 않는 예약을 변경하는 경우, 404를 반환한다.
    - [x] [예외] 지나간 날짜·시간으로 변경하는 경우, 422를 반환한다.
    - [x] [예외] 변경하려는 날짜+시간+테마에 이미 예약이 있는 경우, 409를 반환한다.
- [x] 예약을 삭제한다.
    - [x] 예약 취소 시 해당 슬롯의 첫 번째 대기자가 자동으로 예약으로 전환된다.
    - [x] 대기자가 없는 경우 예약만 삭제된다.
    - [x] [예외] 존재하지 않는 예약을 삭제하는 경우, 404를 반환한다.
    - [x] [예외] 대기자 승격 중 동시 충돌이 발생하는 경우, 409를 반환한다.

### 예약 대기 기능

- [x] 이미 다른 사용자에 의해 예약된 슬롯(날짜+시간+테마)에 대기를 신청할 수 있다.
    - [x] 같은 슬롯에 대한 대기는 신청 순서대로 순번이 부여된다.
    - [x] [예외] 예약이 존재하지 않는 슬롯에 대기를 신청하는 경우, 404를 반환한다.
    - [x] [예외] 본인이 이미 예약한 슬롯에 대기를 신청하는 경우, 409를 반환한다.
    - [x] [예외] 동일한 예약대기자이름+날짜+시간+테마에 이미 예약 대기가 있는 경우, 409를 반환한다.
    - [x] [예외] 지나간 날짜·시간에 대한 대기 신청은 불가능하며, 422를 반환한다.
- [x] 사용자는 본인의 대기를 취소할 수 있다.
    - [x] 대기가 취소되면 나머지 대기자의 순번이 자동으로 재정렬된다.
    - [x] [예외] 존재하지 않는 대기를 취소하는 경우, 404를 반환한다.

### 예외 처리 기능

- [x] 서버 내부 오류 발생 시 500을 반환하며 사용자에게 노출되지 않도록 한다.
- [x] DB 고유 제약 위반(동시 충돌) 발생 시 409를 반환한다.
