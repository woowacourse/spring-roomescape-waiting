...
- 예약(RESERVED)과 대기(WAITING) 목록을 날짜·시간 순으로 함께 반환한다.
...
- [x] 예외: 예약이 존재하지 않는 슬롯에 대기를 신청할 수 없다.
    - 상태 코드: 404
    - 예외 메시지: "존재하지 않는 예약에 대기를 신청할 수 없습니다."
- [x] 예외: 본인이 이미 예약한 슬롯에 대기를 신청할 수 없다.
    - 상태 코드: 409
    - 예외 메시지: "동일한 사용자 이름, 예약 날짜, 시간, 테마에 이미 예약이 존재합니다."
- [x] 예외: 동일한 사용자+날짜+시간+테마에 이미 대기가 존재한다.
    - 상태 코드: 409
    - 예외 메시지: "동일한 사용자 이름, 예약 날짜, 시간, 테마에 이미 예약 대기가 존재합니다."
- [x] 예외: 지나간 날짜·시간에 대한 대기 신청은 불가능하다.
    - 상태 코드: 422
    - 예외 메시지: "지나간 날짜·시간에 대한 예약 생성은 불가능합니다."
...
- [x] 예외: 존재하지 않는 대기를 취소할 수 없다.
    - 상태 코드: 404
    - 예외 메시지: "존재하지 않는 예약 대기입니다."
...
- 예약 취소 시 해당 슬롯의 첫 번째 대기자가 자동으로 예약으로 전환된다. 대기자가 없으면 예약만 삭제된다.
...
- [x] 예외: 존재하지 않는 예약을 삭제할 수 없다.
    - 상태 코드: 404
    - 예외 메시지: "존재하지 않는 예약입니다."
- [x] 예외: 대기자 승격 처리 중 동시 충돌이 발생하면 삭제 전체가 롤백된다.
    - 상태 코드: 409
    - 예외 메시지: "요청을 처리하는 중 충돌이 발생했습니다. 잠시 후 다시 시도해주세요."
