...
- 예약 승인은 기존의 예약이 취소되면 자동으로 되는 것이 아닌, 관리자가 수동으로 승인합니다.
    - **이유**: 매장 직접 가서 웨이팅 하는 방식이면 자동 승인이 당연하다고 생각하지만, 지금처럼 웹을 통해 예약하는 상황에서 자동 승인을 하는 것이 좋지 않다고 생각했습니다.
    - 물론, 예약 정책을 조금 더 추가하면 자동 승인으로 지정할 수 있겠으나, 예약에 대한 추가적인 정책은 지금 단계에서 고려하지 않았습니다.
...
- 현재는 모든 로그인된 회원이 접근이 가능하나, 관리자만 접근 가능하도록 수정 하는게 좋을 것 같다고 생각합니다.
