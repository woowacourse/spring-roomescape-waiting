...
- [ ] 예약이 취소되면 해당 슬롯에 대기가 있을 경우 대기 1번이 예약으로 자동 전환된다.
- [ ] 취소 대상 슬롯에 대기가 없으면 예약만 삭제된다.
- [ ] 대기가 예약으로 전환되면 해당 슬롯의 남은 대기 순번이 재정렬된다.
- [ ] 취소·전환·순번 변화는 하나의 트랜잭션으로 처리되어, 중간 실패 시 전부 롤백된다.
- [ ] 사용자 취소(`/reservations/me/{id}`)와 관리자 삭제(`/reservations/{id}`) 두 경로 모두 동일하게 적용된다.
