...
- [x] 예약을 취소할 때 해당 슬롯에 대기자가 없으면 예약을 삭제한다.
- [x] 예약을 취소할 때 해당 슬롯에 대기자가 있으면 1순위 대기자를 예약자로 자동 전환한다.
- [x] 전환된 대기 데이터는 삭제되고, 남은 대기 순번은 조회 시 자동으로 재정렬된다.
- [x] 예약자 전환(예약 소유자 변경)과 1순위 대기 삭제는 하나의 트랜잭션으로 처리하며, 중간 실패 시 함께 롤백된다.

#### 설계 결정

- **자동 전환 vs 수동 승인 → 자동 전환 선택.** 이 도메인에서 슬롯은 항상 확정 예약 1개가 점유하고 대기는 그 슬롯이 비길 기다림이다. 따라서 "승인"은 슬롯이 빌 때(예약 취소)에만 의미가 있어, 취소 시 1순위 대기를 자동 전환하는 흐름이 가장 자연스럽다고 판단했다.
- **전환 메커니즘 = 소유권 이전.** 예약 행의 소유자(`name`)를 1순위 대기자로 교체하고 그 대기 행을 삭제한다. 슬롯(`reservation.id`)이 유지되므로 남은 대기는 같은 슬롯을 계속 참조하고, 순번은 저장값이 아니라 조회 시 계산되므로 별도 재정렬 작업 없이 자동으로 당겨진다.
- **트랜잭션 경계 = `예약 소유자 변경(UPDATE)` + `1순위 대기 삭제(DELETE)`.** 두 변경은 "슬롯 소유권이 다음 대기자에게 넘어간다"는 단일 사실이므로 함께 커밋되거나 함께 롤백되어야 한다. 일부만 반영되면(예: 소유자만 바뀌고 대기는 남음) 데이터가 깨지므로 `@Transactional`로 묶었다.
- **엣지 케이스:** 대기 없는 취소 → 단순 삭제 / 존재하지 않는 예약 취소 → 404 / 지난 예약 취소 → 차단(사이클1 정책 유지) / 신청 시각이 완전히 같은 대기 → `id` 순으로 1순위 결정. 예약 날짜·시간 *변경*은 대기자가 있으면 여전히 차단한다(전환 대상이 아니므로 사이클1 정책 유지).
...
| 예약 삭제/취소  | `DELETE /reservations/{reservationId}` | - | `204` (대기자가 있으면 1순위 대기가 예약으로 자동 전환) |
| 내 예약 취소    | `POST /pages/user/reservations/{reservationId}/delete` | `reservationName` | `302` (대기자가 있으면 1순위 대기가 예약으로 자동 전환) |
