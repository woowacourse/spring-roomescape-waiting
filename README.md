...
|-------|---------------------|----------|-----------------------|
| RC3-1 | name이 null인 경우      | name     | 예약자명은 필수입니다.          |
| RC3-2 | date가 null인 경우      | date     | 예약 날짜는 필수입니다.         |
| RC3-3 | timeId가 null인 경우    | timeId   | timeId는 필수입니다.        |
| RC3-4 | timeId가 양수가 아닌 경우   | timeId   | timeId의 값이 유효하지 않습니다. |
| RC3-5 | themeId가 null인 경우   | themeId  | themeId는 필수입니다.       |
| RC3-6 | themeId가 양수가 아닌 경우  | themeId  | themeId의 값이 유효하지 않습니다. |
| RC5-1 | timeId가 존재하지 않는 경우  | timeId   | 존재 하지 않는 시간대입니다.      |
| RC5-2 | themeId가 존재하지 않는 경우 | themeId  | 존재 하지 않는 테마입니다.       |

> `name`은 `@NotNull`만 적용되어 null인 경우에만 RC3-1로 처리되며, 공백이거나 20자를 초과하는 값은 도메인 검증(RC4)으로 처리됩니다.
...
| RD5 | 다른 요청이 먼저 같은 예약을 변경한 경우(동시 수정 충돌) | 409 Conflict        | 다른 요청이 먼저 예약을 변경했습니다. 다시 시도해주세요. |
| RD6 | 서버 오류인 경우                   | 500 Internal Server Error | 예상치 못한 오류가 발생했습니다.           |
...
```json
{
  "message": "다른 요청이 먼저 예약을 변경했습니다. 다시 시도해주세요."
}
```
...
4. 삭제되지 않은 예약만 조회합니다.
5. 각 예약에 수정 가능 여부, 취소 가능 여부, 대기 여부, 취소 여부, 수정 권장 여부를 나타내는 status 필드를 포함해야 합니다.
6. 각 예약에 status에 맞는 message 필드를 포함해야 합니다.
7. time, theme 필드에 삭제 여부를 포함해야 합니다.
8. status가 EDITABLE이면 수정 가능, 취소 가능, 수정 권장 아님을 의미합니다.
9. status가 EDIT_RECOMMENDED이면 수정 가능, 취소 가능, 연결된 time 또는 theme가 삭제되어 수정 권장함을 의미합니다.
10. status가 LOCKED이면 수정 불가, 취소 불가, 이미 지난 예약을 의미합니다.
11. status가 CANCELED이면 이미 취소된 예약을 의미합니다.
12. status가 WAITING이면 대기 중인 예약을 의미합니다.
13. waitingNumber는 대기 순번을 의미합니다.
```
...
| RU3 | name이 공백이거나 20자를 초과한 경우                   | 400 Bad Request           | 상태 값이 올바르지 않습니다.   |
| RU4 | 서버 오류인 경우                                 | 500 Internal Server Error | 예상치 못한 오류가 발생했습니다. |

> `name`은 도메인 검증(`ReserverName`)으로 처리되므로 공백·길이 초과 시 `parameterErrors` 없이 상위 메시지만 응답합니다.
...
### `PATCH /api/reservations/{id}` 사용자 예약 변경 API
...
```text
1. 사용자가 예약 ID로 본인의 활성(ACTIVE) 예약 정보를 변경합니다.
2. 요청의 예약자 이름은 기존 예약의 예약자 이름과 일치해야 합니다.
3. 예약 날짜, 예약 시간, 예약 테마를 변경할 수 있습니다.
4. 삭제된 예약은 존재하지 않는 예약으로 처리합니다.
5. 활성(ACTIVE) 상태가 아닌 예약(취소·대기 등)은 변경할 수 없습니다.
...
11. 조회 후 변경을 반영하기 전에 다른 요청이 같은 예약을 먼저 변경하면(동시 수정 충돌) 변경할 수 없습니다.
...
| ID   | 상황                                      | HTTP Code                 | 상위 에러 메시지             |
|------|-----------------------------------------|---------------------------|-----------------------|
| RP1  | 요청 처리 성공                                | 200 OK                    | 없음                    |
| RP2  | 예약 ID의 자료형이 올바르지 않은 경우                  | 400 Bad Request           | 요청 형식이 올바르지 않습니다.     |
| RP3  | 예약 ID의 값이 제약 조건을 만족하지 못한 경우             | 400 Bad Request           | 요청 값이 올바르지 않습니다.      |
| RP4  | 요청 본문 JSON 형식 또는 필드 자료형이 올바르지 않은 경우     | 400 Bad Request           | 요청 형식이 올바르지 않습니다.     |
| RP5  | dto 필드 제약 조건을 만족하지 못한 경우                | 400 Bad Request           | 요청 값이 올바르지 않습니다.      |
| RP6  | 예약자명이 공백이거나 20자를 초과한 경우                 | 400 Bad Request           | 상태 값이 올바르지 않습니다.      |
| RP7  | 변경 내용이 기존 예약과 동일한 경우                    | 400 Bad Request           | 변경할 내용이 없습니다.         |
| RP8  | 조회한 예약의 예약자 이름과 요청 예약자 이름이 다른 경우        | 403 Forbidden             | 예약을 변경할 권한이 없습니다.     |
| RP9  | 예약 ID가 존재하지 않는 경우                       | 404 Not Found             | 예약을 찾을 수 없습니다.        |
...
| RP14 | 다른 요청이 먼저 같은 예약을 변경한 경우(동시 수정 충돌)         | 409 Conflict              | 다른 요청이 먼저 예약을 변경했습니다. 다시 시도해주세요. |
| RP15 | 서버 오류인 경우                               | 500 Internal Server Error | 예상치 못한 오류가 발생했습니다.    |

### 파라미터별 에러 메시지

| ID    | 상황                  | 문제 파라미터명 | 파라미터 에러 메시지          |
|-------|---------------------|----------|----------------------|
| RP3-1 | id의 값이 양수가 아닌 경우    | id       | id의 값은 양수여야 합니다.     |
| RP5-1 | name이 null인 경우      | name     | 예약자명은 필수입니다.         |
| RP5-2 | date가 null인 경우      | date     | 예약 날짜는 필수입니다.        |
| RP5-3 | timeId가 null인 경우    | timeId   | timeId는 필수입니다.       |
| RP5-4 | timeId가 양수가 아닌 경우   | timeId   | timeId의 값이 양수가 아닙니다. |
| RP5-5 | themeId가 null인 경우   | themeId  | themeId는 필수입니다.      |
| RP5-6 | themeId가 양수가 아닌 경우  | themeId  | themeId의 값이 양수가 아닙니다. |

> RP10(수정할 자원 미존재)은 도메인 검증에서 발생하며 `parameterErrors` 없이 상위 메시지만 응답합니다.

### Path Variables

| 이름 | 타입     | 필수 여부 | 설명        | 예시 |
|----|--------|-------|-----------|----|
| id | Number | 필수    | 변경할 예약 ID | 1  |

### Request Body

| 필드      | 타입     | 필수 여부 | 설명                     | 예시         |
|---------|--------|-------|------------------------|------------|
| name    | String | 필수    | 기존 예약자와 일치해야 하는 예약자 이름 | 브라운        |
| date    | Date   | 필수    | 변경할 예약 날짜              | 2026-05-12 |
| timeId  | Number | 필수    | 변경할 예약 시간 ID           | 1          |
| themeId | Number | 필수    | 변경할 예약 테마 ID           | 1          |
...
```json
{
  ...
  "parameterErrors": [
    ...
    {
      "parameter": "timeId",
      "message": "timeId는 필수입니다."
    },
    {
      "parameter": "themeId",
      "message": "themeId는 필수입니다."
    }
  ]
}
```
...
### `PATCH /api/reservations/{id}/cancel?name={name}` 사용자 예약 취소 API
...
```text
1. 사용자가 예약 ID로 본인의 예약을 취소합니다.
2. 요청의 예약자 이름은 기존 예약의 예약자 이름과 일치해야 합니다.
3. 예약 취소는 status를 CANCELED로 변경하는 방식으로 처리합니다.
4. 취소된 예약은 삭제된 예약이 아니므로 이력으로 보존합니다.
...
| ID   | 상황                                        | HTTP Code                 | 상위 에러 메시지          |
|------|-------------------------------------------|---------------------------|--------------------|
| RX1  | 요청 처리 성공                                  | 200 OK                    | 없음                 |
| RX2  | 예약 ID의 자료형이 올바르지 않은 경우                    | 400 Bad Request           | 요청 형식이 올바르지 않습니다.  |
| RX3  | 예약 ID의 값이 제약 조건을 만족하지 못한 경우               | 400 Bad Request           | 요청 값이 올바르지 않습니다.   |
| RX4  | name Query Parameter가 누락된 경우              | 400 Bad Request           | 요청 형식이 올바르지 않습니다.  |
| RX5  | name이 공백이거나 20자를 초과한 경우                   | 400 Bad Request           | 상태 값이 올바르지 않습니다.   |
| RX6  | 조회한 예약의 예약자 이름과 요청 예약자 이름이 다른 경우          | 403 Forbidden             | 예약을 취소할 권한이 없습니다.  |
| RX7  | 예약 ID가 존재하지 않는 경우                         | 404 Not Found             | 예약을 찾을 수 없습니다.     |
| RX8  | 활성(ACTIVE) 예약이 아닌 경우                      | 409 Conflict              | 활성된 예약이 아닙니다.      |
| RX9  | 이미 지난 예약을 취소하려는 경우                        | 409 Conflict              | 지난 예약은 취소할 수 없습니다. |
| RX10 | 서버 오류인 경우                                 | 500 Internal Server Error | 예상치 못한 오류가 발생했습니다. |

### 파라미터별 에러 메시지

| ID    | 상황               | 문제 파라미터명 | 파라미터 에러 메시지      |
|-------|------------------|----------|------------------|
| RX3-1 | id의 값이 양수가 아닌 경우 | id       | id의 값은 양수여야 합니다. |
...
```text
1. 사용자가 예약 ID로 본인의 대기중인 예약을 취소합니다.
2. 요청의 예약자 이름은 기존 예약의 예약자 이름과 일치해야 합니다.
3. 예약 취소는 status를 Status.CANCELED로 변경하는 방식으로 진행합니다.
4. 취소된 예약은 삭제된 예약이 아니므로 이력으로 보존합니다.
5. 대기중인 예약이 아니라면 취소할 수 없습니다.
6. 이미 지난 대기중인 예약은 취소할 수 없습니다.
```
...
| ID   | 상황                                        | HTTP Code                 | 상위 에러 메시지          |
|------|-------------------------------------------|---------------------------|--------------------|
| RY1  | 요청 처리 성공                                  | 200 OK                    | 없음                 |
| RY2  | 예약 ID의 자료형이 올바르지 않은 경우                    | 400 Bad Request           | 요청 형식이 올바르지 않습니다.  |
| RY3  | 예약 ID의 값이 제약 조건을 만족하지 못한 경우               | 400 Bad Request           | 요청 값이 올바르지 않습니다.   |
| RY4  | name Query Parameter가 누락된 경우              | 400 Bad Request           | 요청 형식이 올바르지 않습니다.  |
| RY5  | name이 공백이거나 20자를 초과한 경우                   | 400 Bad Request           | 상태 값이 올바르지 않습니다.   |
| RY6  | 조회한 예약의 예약자 이름과 요청 예약자 이름이 다른 경우          | 403 Forbidden             | 예약을 취소할 권한이 없습니다.  |
| RY7  | 예약 ID가 존재하지 않는 경우                         | 404 Not Found             | 예약을 찾을 수 없습니다.     |
| RY8  | 대기중인 예약이 아닌 경우                            | 409 Conflict              | 대기중인 예약이 아닙니다.     |
| RY9  | 이미 지난 대기중인 예약을 취소하려는 경우                   | 409 Conflict              | 지난 예약은 취소할 수 없습니다. |
| RY10 | 서버 오류인 경우                                 | 500 Internal Server Error | 예상치 못한 오류가 발생했습니다. |

### 파라미터별 에러 메시지

| ID    | 상황               | 문제 파라미터명 | 파라미터 에러 메시지      |
|-------|------------------|----------|------------------|
| RY3-1 | id의 값이 양수가 아닌 경우 | id       | id의 값은 양수여야 합니다. |
...
### `POST /api/reservations/waitings` 사용자 예약 대기 API
...
```text
1. 대기(WAITING) 상태의 예약을 생성합니다.
2. 삭제된 예약 시간 또는 삭제된 테마로는 예약 대기를 생성할 수 없습니다.
3. 현재보다 과거의 날짜/시간으로는 예약 대기를 생성할 수 없습니다.
4. 같은 이름, 날짜, 시간, 테마로 대기중인 예약이 이미 있으면 생성할 수 없습니다.
5. 같은 이름, 날짜, 시간, 테마로 활성 예약이 이미 있으면 생성할 수 없습니다.
6. 해당 날짜, 시간, 테마에 활성 예약이 존재하지 않으면 예약 대기를 생성할 수 없습니다.
```
...
| ID   | 상황                                                  | HTTP Code                 | 상위 에러 메시지                   |
|------|-----------------------------------------------------|---------------------------|-----------------------------|
| RW1  | 요청 처리 성공                                            | 201 Created               | 없음                          |
| RW2  | 요청 본문 JSON 형식 또는 필드 자료형이 올바르지 않은 경우                 | 400 Bad Request           | 요청 형식이 올바르지 않습니다.           |
| RW3  | dto 필드 제약 조건을 만족하지 못한 경우                            | 400 Bad Request           | 요청 값이 올바르지 않습니다.            |
| RW4  | 예약자명이 공백이거나 20자를 초과한 경우                             | 400 Bad Request           | 상태 값이 올바르지 않습니다.            |
| RW5  | 참조 자원이 존재하지 않는 경우                                   | 404 Not Found             | 조회할 자원이 존재하지 않습니다.          |
| RW6  | 이미 지난 날짜/시간으로 대기하려는 경우                              | 409 Conflict              | 지난 예약은 생성할 수 없습니다           |
| RW7  | name, date, timeId, themeId에 해당하는 대기 예약이 이미 존재하는 경우 | 409 Conflict              | 이미 대기 중인 이름, 날짜, 시간, 테마입니다. |
| RW8  | name, date, timeId, themeId에 해당하는 활성 예약이 이미 존재하는 경우 | 409 Conflict              | 이미 예약된 날짜, 시간, 테마입니다.       |
| RW9  | 해당 날짜, 시간, 테마에 활성 예약이 존재하지 않는 경우                    | 409 Conflict              | 아직 예약되지 않은 날짜, 시간, 테마입니다.   |
| RW10 | 서버 오류인 경우                                           | 500 Internal Server Error | 예상치 못한 오류가 발생했습니다.          |

### 파라미터별 에러 메시지

| ID    | 상황                  | 문제 파라미터명 | 파라미터 에러 메시지           |
|-------|---------------------|----------|-----------------------|
| RW3-1 | name이 null인 경우      | name     | 예약자명은 필수입니다.          |
| RW3-2 | date가 null인 경우      | date     | 예약 날짜는 필수입니다.         |
| RW3-3 | timeId가 null인 경우    | timeId   | timeId는 필수입니다.        |
| RW3-4 | timeId가 양수가 아닌 경우   | timeId   | timeId의 값이 유효하지 않습니다. |
| RW3-5 | themeId가 null인 경우   | themeId  | themeId는 필수입니다.       |
| RW3-6 | themeId가 양수가 아닌 경우  | themeId  | themeId의 값이 유효하지 않습니다. |
| RW5-1 | timeId가 존재하지 않는 경우  | timeId   | 존재 하지 않는 시간대입니다.      |
| RW5-2 | themeId가 존재하지 않는 경우 | themeId  | 존재 하지 않는 테마입니다.       |
...
```json
{
  "message": "이미 대기 중인 이름, 날짜, 시간, 테마입니다."
}
```
...
```json
{
  "message": "아직 예약되지 않은 날짜, 시간, 테마입니다."
}
```
...
### `GET /api/times?date={date}&themeId={themeId}` 예약 시간 가능 여부 조회 API
...
```text
1. 특정 날짜와 테마 기준으로 예약 시간별 가능 여부를 조회합니다.
2. 삭제된 테마로는 예약 가능 시간을 조회할 수 없습니다.
3. 삭제되지 않은 예약 시간만 응답에 포함합니다.
4. 삭제되지 않고 취소되지 않은 활성 예약이 존재하는 시간은 `available`을 `false`로 반환합니다.
5. 삭제된 예약은 예약 점유로 보지 않습니다.
6. 취소된 예약은 예약 점유로 보지 않습니다.
7. 대기중인 예약은 예약 점유로 보지 않습니다.
8. 삭제된 예약 시간에 연결된 기존 예약은 예약 가능 여부 계산에 반영하지 않습니다.
9. 예약 가능한 시간이 없어도 삭제되지 않은 예약 시간은 `available`을 `false`로 포함해 반환합니다.
10. 삭제되지 않은 예약 시간이 없으면 빈 목록을 반환합니다.
```
...
| TA2 | Query Parameter가 누락되었거나 형식이 올바르지 않은 경우 | 400 Bad Request           | 요청 형식이 올바르지 않습니다.  |
...
| HC3 | name, description, imageUrl 중 null인 필드가 있는 경우 | 400 Bad Request   | 요청 값이 올바르지 않습니다.                  |
| HC4 | name이 공백이거나 255자를 초과한 경우           | 400 Bad Request           | 테마 이름은 필수이며 255자 이하여야 합니다.        |
| HC5 | description이 공백이거나 255자를 초과한 경우     | 400 Bad Request           | 테마 설명은 필수이며 255자 이하여야 합니다.        |
| HC6 | imageUrl이 공백·2000자 초과·URL 형식 위반인 경우 | 400 Bad Request           | 테마 이미지 URL은 필수이며 2000자 이하의 올바른 URL 형식이어야 합니다. |
| HC7 | 같은 name의 테마가 이미 존재하는 경우             | 409 Conflict              | 이미 등록된 테마입니다.                     |
| HC8 | 서버 오류인 경우                           | 500 Internal Server Error | 예상치 못한 오류가 발생했습니다.                |

### 파라미터별 에러 메시지

| ID    | 상황                  | 문제 파라미터명    | 파라미터 에러 메시지       |
|-------|---------------------|-------------|-------------------|
| HC3-1 | name이 null인 경우      | name        | 테마 이름은 필수입니다.     |
| HC3-2 | description이 null인 경우 | description | 테마 설명은 필수입니다.     |
| HC3-3 | imageUrl이 null인 경우  | imageUrl    | 테마 이미지 URL은 필수입니다. |

> name·description·imageUrl의 공백, 길이 초과, URL 형식 위반은 도메인 검증(HC4~HC6)에서 처리되며 `parameterErrors` 없이 상위 메시지만 응답합니다.
...
```json
{
  "message": "테마 이름은 필수이며 255자 이하여야 합니다."
}
```
...
```json
{
  "message": "테마 설명은 필수이며 255자 이하여야 합니다."
}
```

#### HC6 테마 이미지 URL 검증 실패

```json
{
  "message": "테마 이미지 URL은 필수이며 2000자 이하의 올바른 URL 형식이어야 합니다."
}
```
...
현재 예외 처리는 웹 API 중심의 정책으로 가져갑니다.

- 도메인별 ErrorType은 비즈니스 오류 식별자, 사용자에게 보여줄 메시지, HTTP Status를 함께 관리합니다.
- 예외가 발생한 지점과 API 응답으로 내려갈 메시지 및 상태 코드를 가깝게 두어 흐름을 쉽게 파악할 수 있도록 합니다.
- 최상위 에러 응답은 `message`를 기본으로 사용합니다.
- 클라이언트가 한 번에 고칠 수 있는 필드 목록이 명확한 경우에만 `parameterErrors`를 함께 내려줍니다.
- JSON 파싱 실패, 타입 불일치, 필수 Query Parameter 누락처럼 필드 단위로 안정적으로 나누기 어려운 오류는 `message`만 내려줍니다.
