...
확정 예약이 없는 슬롯에 대기만 남는 상태는 허용하지 않는다.
예약 취소 또는 예약 변경으로 슬롯이 비면, 같은 트랜잭션 안에서 첫 번째 대기를 예약으로 승격하고 해당 대기를 삭제한다.
다만 복수 요청이 동시에 들어오는 상황까지 완전히 제어하는 row lock, 격리 수준 조정 등은 현재 레벨 범위를 넘어선다고 판단해 적용하지 않았다.
