...
- [ ] 스키마 추가 `table waiting`
- [ ] Command 객체 추가 - `class WaitingCommand`
- [ ] 레포지토리 추가 `class WaitingRepository`
  - [ ] 이름으로 순번 조회 `int calculateWaitingNumberByName(WaitingCommand waiting)`
  - [ ] 삽입 `void insert(WaitingCommand waiting)`
  - [ ] 삭제 `void delete(WaitingCommand waiting)`
- [ ] 서비스 추가
  - [ ] 이름으로 순번 조회 `int waitingNumber(WaitingCommand waiting)`
  - [ ] 삽입 `void applyWaiting(WaitingCommand waiting)`
  - [ ] 삭제 `void cancelWaiting(WaitingCommand waiting)`
- [ ] Request 객체 추가 - `class WaitingRequest`
- [ ] 컨트롤러 추가
  - [ ] 이름으로 순번 조회 `int waitingNumber(@RequestBody WaitingRequest waitingRequest)`
