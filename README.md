...
### 3단계 - 내 예약 조회/변경/취소
- [X] 사용자 본인 예약 조회 API 구현
- [X] 사용자 본인 예약 취소 API 구현
- [X] 예약 취소 권한 검증 적용
- [X] 사용자 본인 예약 변경 API 구현
- [X] 예약 변경 권한 검증 적용
- [X] 지난 예약 변경·취소 예외 응답 적용
- [X] 예약 변경 중복 예외 응답 적용
- [X] 기존 날짜·시간과 동일한 예약 변경 예외 응답 적용
- [X] 내 예약 조회·변경·취소 테스트 추가
- [X] 내 예약 조회 화면 추가
- [X] 내 예약 변경·취소 화면 추가
...
### 1. 규칙을 적용해서 변경한 코드 1곳 이상

#### Rule 6. 비즈니스 규칙 위반 시 응답 기준 적용

에러 응답은 문자열 대신 `{code, detail}` 구조로 통일했습니다.

`code`는 클라이언트가 분기하기 위한 안정적인 계약이고,
`detail`은 API 소비자가 실패 원인을 이해하기 위한 설명이라고 정리했습니다.

최종 사용자에게 어떤 문구를 보여줄지는 클라이언트의 책임이지만,
서버는 클라이언트가 다음 처리를 판단할 수 있도록
HTTP 상태 코드, code, detail을 일관되게 제공해야 한다고 보았습니다.

그래서 비즈니스 규칙 위반 상황마다 고정된 error code를 두고,
화면에서도 문자열이 아니라 `detail`을 읽어 표시하도록 수정했습니다.


#### Rule 3. 관리자/사용자 API 분리 기준 적용

처음에는 같은 예약 리소스를 생성하는 행위라면
관리자와 사용자가 같은 API와 로직을 사용해도 된다고 생각했습니다.

하지만 구현하면서 같은 예약 생성이라도
사용자 예약과 관리자 예약은 사용 맥락과 정책이 다를 수 있다고 판단했습니다.

예를 들어 사용자는 과거 예약 생성이 제한되지만,
관리자는 현장 상황상 과거 시간 예약을 대신 등록해야 할 수 있습니다.

그래서 관리자 API는 `/admin` prefix로 분리하고,
컨트롤러 패키지도 `admin`, `user`, `view`로 나누었습니다.

다만 중복 예약 검증처럼 공통으로 적용되어야 하는 정책은 유지하고,
공통 저장 흐름은 private 메서드로 묶어 중복을 줄였습니다.


### 2. 변경/취소에서 발견한 엣지 케이스와 처리 방향

예약 변경/취소에서는 “과거 예약”과 관련된 경우를 나누어 처리했습니다.

새로 요청한 날짜·시간이 과거인 경우는 요청값 자체가 잘못된 것이므로 `PAST_RESERVATION` 400으로 처리했습니다.
반면 기존 예약 자체가 이미 지난 경우는 리소스 상태상 더 이상 변경하거나 취소할 수 없는 상황이므로 `PAST_RESERVATION_LOCKED` 409로 분리했습니다.

계산 로직은 비슷해 보여도, 실패 원인과 정책 의미가 다르다면 별도 error code로 분리하는 편이 낫다고 판단했습니다.

또한 예약 변경 시 기존 날짜·시간과 동일한 값으로 변경 요청하는 경우도 별도로 처리했습니다.

이를 중복 예약 검증에 맡기면 자기 자신을 중복 예약으로 판단할 수 있었기 때문입니다.
그래서 동일한 변경 요청은 `UNCHANGED_RESERVATION`으로 먼저 거부하고, 이후 실제 중복 예약 여부를 검사하도록 정리했습니다.


### 3. 지금까지의 규칙 중 유지/수정/폐기한 항목

#### 수정한 규칙

##### Rule 3. 관리자/사용자 API 분리 기준

> **(If-Then)** 만약 리소스의 식별자는 동일하지만, 사용자 권한이나 사용 맥락(Context)에 따라 정책과 허용 범위가 달라진다면 → 관리자와 사용자 API를 분리하거나 정책 로직을 별도로 둔다.

변경 이유:
처음에는 같은 예약 리소스를 다루면 같은 API와 Service를 사용하는 방향에 가까웠습니다.
하지만 구현하면서 같은 예약 생성이라도 관리자와 사용자는 정책과 사용 맥락이 다를 수 있다고 판단했습니다.

예를 들어 사용자는 과거 예약 생성이 제한되지만,
관리자는 현장 상황상 과거 시간 예약을 대신 등록해야 할 수 있었습니다.

따라서 현재는 리소스 동일 여부뿐 아니라, 조회/수정 목적과 정책 Context도 함께 고려해야 한다고 수정했습니다.


##### Rule 6. 비즈니스 규칙 위반 시 응답과 “다음 행동"을 담는 기준

> **(If-Then)** 만약 비즈니스 규칙을 위반한 요청이라면 → 4xx 상태 코드와 함께, 클라이언트가 실패 원인을 안정적으로 식별할 수 있는 code와 원인 이해를 위한 detail을 제공한다.
>
> **(원칙)** code는 클라이언트가 분기하기 위한 안정적인 계약으로 사용하고, detail은 API 소비자가 실패 원인을 이해할 수 있도록 설명한다.
>
> **(원칙)** 서버는 최종 사용자에게 보여줄 문구를 직접 결정하기보다, 클라이언트가 다음 처리를 판단할 수 있는 단서를 제공한다.

변경 이유:
처음에는 `detail`을 사용자가 바로 보는 메시지에 가깝게 생각했습니다.
하지만 구현하면서 서버의 직접 대상은 최종 사용자가 아니라 API 클라이언트라는 관점으로 정리되었습니다.

따라서 서버는 UX 문구 자체를 책임지기보다,
클라이언트가 실패 원인과 처리 방향을 판단할 수 있도록 일관된 상태 코드와 error code, 설명 정보를 제공하는 방향이 더 적절하다고 판단했습니다.


그 외 규칙은 현재 기준을 유지할 예정입니다.
...
- 그 명사 중 무엇을 필터/표현 조건으로 사용할 것인지

API 명세 설계에 대한 각자의 관점에 맞춰 판단한다.
...
- **(If-Then)** 만약 판단에 필요한 원천 데이터가 서버에만 존재한다면 → 해당 로직은 서버가 책임지고 결과값을 내려준다.
  - 예: 예약 가능 여부(`available`)는 서버가 기존 예약 현황을 계산하여 응답에 포함한다.
- **(If-Then)** 만약 클라이언트가 받은 데이터를 가공 없이 화면에 그리는 역할이라면 → 서버는 클라이언트가 바로 사용할 수 있는 형태로 응답을 설계한다.
- **(우선순위)** 책임 판단 순서
  1. 데이터의 무결성을 보장해야 하는 곳이 어디인가?
  2. 클라이언트가 중복된 계산 로직을 가질 필요가 있는가?
- **(금지)** 클라이언트에게 비즈니스 로직 판단을 떠넘기지 않는다.
  - 이유: 데이터 불일치 문제를 방지하고 클라이언트의 복잡도를 낮추기 위함이다.
...
- 특징:
  - 직관성: URL(리소스 명사)과 HTTP 메서드(행위 동사)의 조합만으로 해당 API가 어떤 자원에 대해 무슨 작업을 수행하는지 누구나 명확히 추론할 수 있다.
  - 명확성: 리소스의 정의, 메서드 시그니처 등에 대해 팀이 합의한 명확한 기준과 규칙을 따른다.
  - 문서화의 역할: 잘 짜인 API 명세 자체가 설명서 역할을 하여, 개발자 간의 소통 비용과 유지보수 비용을 낮춘다.
...
### 6. 비즈니스 규칙 위반 시 응답과 “다음 행동"을 담는 기준

- **(If-Then)** 만약 비즈니스 규칙을 위반한 요청이라면 → 4xx 상태 코드와 함께, 클라이언트가 실패 원인을 안정적으로 식별할 수 있는 code와 원인 이해를 위한 detail을 제공한다.
- **(원칙)** code는 클라이언트가 분기하기 위한 안정적인 계약으로 사용한다.
- **(원칙)** detail은 API 소비자가 실패 원인을 이해할 수 있도록 설명한다.
- **(원칙)** 서버는 최종 사용자에게 보여줄 문구를 직접 결정하기보다, 클라이언트가 다음 처리를 판단할 수 있는 단서를 제공한다.
- **(금지)** 서버가 최종 사용자에게 특정 행동을 직접 지시하는 UX 문구를 강제하지 않는다.
