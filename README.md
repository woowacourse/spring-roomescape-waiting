...
### 예약자 이름 입력 시 영어 사용 필수
현재 시스템은 인증 처리를 위해 HTTP `Authorization` 헤더를 사용하고 있습니다. **HTTP/1.1 규약 상 헤더 값에는 ISO-8859-1 (ASCII) 문자만 허용**됩니다.

프론트엔드 단에서 별도의 URL 인코딩 처리가 되어있지 않으므로, 예약 생성 후 변경이나 취소를 테스트할 때 한글 이름(예: `브라운`)을 입력하면 브라우저에서 `String contains non ISO-8859-1 code point` 에러가 발생하며 요청이 차단됩니다.

원활한 테스트를 위해 **예약자 및 인증용 이름은 반드시 영어(예: `Brown`, `Cony`)로 입력**해 주시기 바랍니다.
