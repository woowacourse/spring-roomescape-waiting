...
### 예약 대기 기능
- [X] 날짜/테마/예약 가능 시간대를 선택해 예약 대기를 저장할 수 있다.
  - [X] 만약 해당 조건에 대한 예약이 존재하지 않는 경우 예외가 발생한다.
- [ ] 예약 대기를 취소할 수 있다.
- [X] 사용자별 예약 목록 조회 시, 기존 예약 목록 외에 예약 대기를 걸어둔 항목들도 반환한다.
  - [X] 이때 대기 순번이 몇 번인지도 알려준다.
- [ ] 날짜/테마/예약 가능 시간대에 대해 특정 사용자의 예약이 존재하면 예약을 생성할 수 없다.
- [ ] 날짜/테마/예약 가능 시간대에 대해 특정 사용자의 예약이 존재하면 해당 사용자는 예약 대기를 생성할 수 없다.
- [ ] 날짜/테마/예약 가능 시간대에 대해 특정 사용자의 예약 대기가 존재하면 해당 사용자는 예약 대기를 생성할 수 없다.

### 예약 대기 관리
- [ ] 조회 시간 이전의 예약 대기 목록을 제외한 예약 대기 목록을 아래 조건에 맞게 반환한다.
  - 1. 날짜 오름차순 (1순위)
  - 2. 예약 가능 시간대 오름차순 (2순위)
  - [ ] 만약 관리자 권한이 없는 경우, 예외가 발생한다.
- [ ] 예약 취소가 발생하는 경우, 해당 날짜/테마/예약 가능 시간대에 가장 먼저 예약 대기를 한 사용자에 대한 예약을 생성한다.
  - [ ] 예약이 된 예약 대기 항목은 삭제한다.
- [ ] 예약 대기를 거절할 경우, 해당 예약 대기는 삭제된다.
  - [ ] 만약 관리자 권한이 없는 경우, 예외가 발생한다.
