...
## 방탈출 예약 대기 승인 cycle 2 : 구현 기능 목록

### 1단계 - 예약 대기 승인
- [x] **대기 순번 정렬 기준 보완**
  - `ORDER BY created_at ASC` 에 `id ASC` 를 타임브레이커로 추가한다.
  - 동시 신청 시에도 먼저 INSERT된 행이 항상 앞 순번을 가진다.
- [x] **예약 취소 시 첫 번째 대기자 자동 승인**
  - 예약 취소 시 동일 슬롯(날짜+시간+테마)의 대기 순번 1번이 자동으로 확정 예약으로 전환된다.
  - 취소와 승인은 하나의 트랜잭션으로 처리된다.
  - 대기자가 없으면 그냥 삭제만 한다.
- [x] **예약 변경 시 원래 슬롯 대기자 자동 승인**
