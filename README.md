...
- [ ] 예약이 이미 존재하는 날짜, 테마, 시간에 대해 예약 대기를 신청할 수 있다.
- [ ] 예약이 가능한 시간에는 대기 신청이 아니라 기존 예약 생성 흐름을 사용한다.
- [ ] 예약 가능 시간 조회 시 예약 완료 여부와 대기 가능 여부를 구분할 수 있다.
- [ ] 대기 신청은 신청자 이름과 대기 요청 시간을 기록한다.
- [ ] 대기 신청은 기존 예약을 참조하는 별도 대기 테이블에 저장한다.
- [ ] 같은 예약에 대해 이미 내 이름으로 대기가 있으면 다시 대기할 수 없다.
- [ ] 같은 사람이 더블 클릭 등으로 동시에 대기 신청해도 중복 대기가 생성되지 않는다.
- [ ] 같은 예약에 여러 명이 대기하면 요청 시간이 빠른 순서대로 순번을 부여한다.
- [ ] 완전히 같은 시간에 대기 신청이 들어와도 순번이 일관되게 결정된다.
- [ ] 내 대기 순번을 조회할 수 있다.
- [ ] 중간 순번의 대기가 취소되면 뒤 순번이 당겨져 조회된다.
