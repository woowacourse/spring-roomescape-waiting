...
- [x] 스키마 수정 `table waiting`
    - [x] id 추가
    - [x] 기존 기본키 UNIQUE
- [ ] 예약/대기 정보를 모두 담을 Response 객체 `class ReservationAndWaiting`
    - [ ] 기존 Waiting 필드들..
    - [ ] 예약 상태를 담을 `boolean reserved`
    - [ ] 대기 순번을 담을 `int waitingNumber`
- [ ] 대기 정보를 담을 도메인 객체 `class Waiting`
    - [ ] Command 객체 제거 - `class WaitingCommand`
    - [ ] Waiting 생성 체이닝 대체
    - [ ] findByName 수정
- [ ] 예약/대기 정보를 모두 담아 반환할 메서드 `getReservationByName` 확장
    - 이름을 기준으로 `예약` 만 반환하던 메서드를
    - 이름을 기준으로 `예약` 과 `대기` 정보 모두를 반환하는 메서드로

`ReservationController`

- [ ] 이름을 RequestParam 으로 받아 해당 이름으로 예약/대기 정보를 조회할 메서드 `ResponseEntity<ReservationAndWaiting> getReservationByName`

`WaitingService`

- [ ] 이름을 기준으로 대기 정보를 조회할 메서드 `List<Waiting> findWaitingByName(String name)`

`WaitingRepository`

- [ ] 이름을 기준으로 대기 정보를 조회할 메서드 `List<Waiting> findByName(String name)`
