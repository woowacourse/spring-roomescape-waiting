...
- 취소 후 같은 슬롯의 예약 확정자와 대기 순번은 남아 있는 활성 요청의 신청 순서 기준으로 다시 계산된다.
...
- 이미 지난 예약 또는 대기는 취소하거나 변경할 수 없다.
- 예약 요청을 변경할 때 현재 요청과 같은 날짜와 시간으로 변경할 수 없다.
...
예약 또는 대기를 취소하면 기존 요청은 `reservation_history`에 복사한 뒤 `reservation`에서 제거한다.
...
| statusCode | 상황 |
| --- | --- |
| 400 Bad Request | 요청 본문, 쿼리 파라미터, path variable의 형식이 잘못된 경우, 필수값이 누락된 경우, 빈 문자열이나 양수가 아닌 id처럼 유효하지 않은 값이 전달된 경우, 지나간 날짜와 시간으로 예약 생성/변경을 요청한 경우, 이미 지난 예약 또는 대기를 취소/변경하려는 경우 |
...
예약과 대기를 함께 반환한다.
`id`는 활성 예약 요청 id를 의미한다.
`waitingRank`는 예약 확정 항목에서는 `null`이고, 대기 항목에서는 현재 대기 순번이다.
...
| statusCode | 상황 |
| --- | --- |
| 200 OK | 예약자 이름에 해당하는 활성 예약 및 대기 목록 조회 성공 |
...
| 400 Bad Request | 예약 id 또는 `timeId`가 양수가 아닌 경우, 예약자 이름이 비어 있거나 255자를 초과한 경우, 날짜 형식이 `yyyy-MM-dd`가 아닌 경우, 필수값이 누락된 경우, 이미 지난 요청을 변경하려는 경우, 지나간 날짜와 시간으로 변경하려는 경우, 현재 요청과 같은 날짜와 시간으로 변경하려는 경우 |
...
| statusCode | 상황 |
| --- | --- |
| 204 No Content | 본인 예약 또는 대기 취소 성공 |
| 400 Bad Request | 예약 요청 id가 양수가 아닌 경우, `name` 쿼리 파라미터가 누락되었거나 비어 있는 경우, 이미 지난 요청을 취소하려는 경우 |
| 404 Not Found | 해당 이름으로 활성 예약 요청을 찾을 수 없거나 삭제 시점에 요청이 존재하지 않는 경우 |
...
관리자 취소도 사용자 취소와 동일하게 취소 이력으로 이동한 뒤 활성 예약 목록에서 제거한다.
...
| statusCode | 상황 |
| --- | --- |
| 204 No Content | 예약 요청 취소 성공 |
| 400 Bad Request | 예약 요청 id가 양수가 아닌 경우 |
| 404 Not Found | 취소하려는 활성 예약 요청이 존재하지 않는 경우 |
...
- 시간 목록에는 예약 가능 상태와 대기 가능 상태가 함께 표시된다.
- 예약 가능한 시간은 예약 신청 버튼을 제공한다.
- 이미 예약된 시간은 대기 신청 버튼을 제공한다.
- 예약과 대기는 모두 `POST /reservations` 요청으로 생성된다.
- 사용자는 이름을 입력해 본인의 예약과 대기를 함께 조회할 수 있다.
- 내 예약 목록에는 `예약 확정`, `대기 n번째`처럼 상태와 대기 순번이 표시된다.
...
- 관리자는 활성 예약 요청 목록을 조회하고 요청을 취소할 수 있다.
