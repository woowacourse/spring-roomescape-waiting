...
- 비활성화된 예약 시간과 테마는 신규 예약에서 선택할 수 없다.
...
- 예약 요청 변경 시 날짜와 시간이 기존 요청과 같으면 기존 활성 요청을 그대로 반환한다.
- 예약 요청 변경 성공 시 응답되는 `id`는 실제 변경이 발생한 경우 새로 생성된 예약 요청 id이며, 기존 예약 요청 id와 다를 수 있다.
...
| 409 Conflict               | 이미 존재하는 자원을 생성하려는 경우, 같은 사용자가 같은 슬롯에 활성 요청을 중복 생성하려는 경우                                                                                         |
...
단, 요청한 날짜와 시간이 기존 요청과 같으면 취소 이력이나 새 요청을 생성하지 않고 기존 활성 요청을 그대로 반환한다.
변경 대상 슬롯에 활성 요청이 없으면 예약 확정으로 생성되고, 이미 활성 요청이 있으면 대기로 생성된다.
단, 같은 사용자가 변경 대상 슬롯에 이미 다른 활성 예약 또는 대기를 가지고 있으면 중복 신청으로 거부한다.
실제 변경이 발생한 경우 변경 성공 응답의 `id`는 변경 전 예약 요청 id가 아니라 새로 생성된 예약 요청 id이다.
따라서 클라이언트는 변경 이후 작업에 응답으로 받은 `id`를 사용해야 한다.
...
응답의 `id`는 실제 변경이 발생한 경우 변경 결과로 새로 생성된 예약 요청 id이다.
요청한 날짜와 시간이 기존 요청과 같으면 기존 예약 요청 id가 응답된다.
...
| statusCode      | 상황                                      |
|-----------------|-----------------------------------------|
| 204 No Content  | 예약 시간 비활성화 성공, 또는 이미 비활성화된 경우          |
| 400 Bad Request | 예약 시간 id가 양수가 아닌 경우, 상태가 누락되었거나 유효하지 않은 경우 |
...
| statusCode      | 상황                                |
|-----------------|-----------------------------------|
| 204 No Content  | 테마 비활성화 성공, 또는 이미 비활성화된 경우        |
| 400 Bad Request | 테마 id가 양수가 아닌 경우, 상태가 누락되었거나 유효하지 않은 경우 |
