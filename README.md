...
## 사이클2 1단계 - 예약 대기 자동 승인(전환)

- [x] 예약이 취소되면 해당 슬롯에 대기가 있을 경우 대기 1번이 예약으로 자동 전환된다.
- [x] 취소 대상 슬롯에 대기가 없으면 예약만 삭제된다.
- [x] 대기가 예약으로 전환되면 해당 슬롯의 남은 대기 순번이 재정렬된다.
- [x] 취소·전환·순번 변화는 하나의 트랜잭션으로 처리되어, 중간 실패 시 전부 롤백된다.
- [x] 사용자 취소(`/reservations/me/{id}`)와 관리자 삭제(`/reservations/{id}`)에서 대기가 있으면 동일하게 대기 1번이 자동 전환된다.
- [x] 지난 예약은 누구도 취소·삭제할 수 없고, 지난 슬롯의 대기도 취소할 수 없다(과거 동결).
...
### 예약 취소 시 대기 자동 전환 (사이클2)

`DELETE /reservations/{id}`(관리자)와 `DELETE /reservations/me/{id}`(사용자) 모두, 취소 대상 예약이 점유한 슬롯에 대기가 남아 있으면 가장 앞 순번(`created_at`, `id` 순) 대기 1명을 그 슬롯의 예약으로 전환한 뒤 `204`를 반환한다. 대기가 없으면 예약만 삭제한다. 전환과 남은 대기 순번 재정렬은 취소와 한 트랜잭션으로 묶여, 중간 실패 시 모두 롤백된다. 단, 지난 예약은 사용자 취소·관리자 삭제 모두 `422`로 거부하고, 지난 슬롯의 대기 취소도 `422`로 거부한다(과거 데이터는 변경 불가).
