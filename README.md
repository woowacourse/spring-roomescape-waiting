...
- [x] 사용자 취소(`/reservations/me/{id}`)와 관리자 삭제(`/reservations/{id}`) 두 경로 모두 동일하게 적용된다.
...
`DELETE /reservations/{id}`(관리자)와 `DELETE /reservations/me/{id}`(사용자) 모두, 취소 대상 예약이 점유한 슬롯에 대기가 남아 있으면 가장 앞 순번(`created_at`, `id` 순) 대기 1명을 그 슬롯의 예약으로 전환한 뒤 `204`를 반환한다. 대기가 없으면 예약만 삭제한다. 전환과 남은 대기 순번 재정렬은 취소와 한 트랜잭션으로 묶여, 중간 실패 시 모두 롤백된다.
