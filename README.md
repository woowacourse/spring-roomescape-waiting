...
### 정책
- 본 기능은 수동 승인 방식을 사용한다.
- 예약 취소 시 대기번호 1번은 자동으로 예약으로 전환되지 않는다.
- 예약 취소 시 대기번호 1번을 승격 대상으로 지정하며, 관리자의 승인 이후 예약으로 전환된다.

## 예약 취소

- [ ] 예약이 취소되면 대기번호 1번 대기가 자동으로 승격 대상이 된다.
- [ ] 승격 대상이 존재하는 동안 해당 슬롯에는 예약을 생성할 수 없다.
- [ ] 승격 대상이 존재하는 동안 새로운 대기 등록은 가능하다.
- [ ] 하나의 슬롯에는 동시에 하나의 승격 대상만 존재할 수 있다.

## 승격 대상 처리

- [ ] 관리자는 승격 대상을 승인하거나 거절할 수 있다.
  - [ ] 승인 시 해당 대기는 예약으로 승격된다.
  - [ ] 거절 시 해당 대기는 취소 처리된다.

## 대기 순번 관리

- [ ] 대기번호 1번이 예약으로 전환되면 해당 대기를 대기열에서 제거한다.
- [ ] 대기열에서 제거된 대기 뒤에 존재하는 대기의 순번을 재정렬한다.
- [ ] 대기가 취소되면 취소된 대기 뒤에 존재하는 대기의 순번을 재정렬한다.
- [ ] 승격 대상이 거절되면 해당 대기를 대기열에서 제거하고 순번을 재정렬한다.

## 승격 대상 재지정

- [ ] 승격 대상이 예약으로 전환되거나 취소 또는 거절된 경우 대기열이 존재하면 새로운 대기번호 1번을 승격 대상으로 지정한다.
- [ ] 대기열이 존재하지 않는 경우 승격 대상은 생성되지 않는다.
