...
| RP9  | 이미 취소된 예약을 변경하려는 경우                     | 409 Conflict              | 활성된 예약이 아닙니다.         |
...
2. 요청의 예약자 이름은 기존 예약의 예약자 이름과 일치해야 합니다.
3. 예약 취소는 canceled_at에 현재 시각을 기록하는 상태 변경으로 처리합니다.
4. 취소된 예약은 삭제된 예약이 아니므로 이력으로 보존합니다.
5. 이미 취소된 예약은 다시 취소할 수 없습니다.
...
| RX8  | 이미 취소된 예약을 다시 취소하려는 경우                    | 409 Conflict              | 이미 취소된 예약입니다.      |
...
| RW5 | name, date, timeId, themeId에 해당하는 예약이 존재하는 경우 | 409 Conflict              | 이미 대기 중인 이름, 날짜, 시간, 테마입니다. |
| RW6 | 서버 오류인 경우                                     | 500 Internal Server Error | 예상치 못한 오류가 발생했습니다.          |
